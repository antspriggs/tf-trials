...
export default function TimerPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-gray-900 border-b border-gray-800">
        ...
        <div className="px-4 pt-3 pb-2">
          <select
            ...
            className="w-full bg-gray-800 text-white border border-gray-700 rounded-lg px-3 py-2.5 text-base font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
          >
        ...
        <div className="px-4 py-2 text-center">
          <div
            className="text-5xl sm:text-7xl font-mono tracking-wider"
            style={{ fontVariantNumeric: 'tabular-nums' }}
          >
            {displayTime}
          </div>
          {selectedEventName && timerState !== 'idle' && (
            <div className="text-sm text-gray-400 mt-1">{selectedEventName}</div>
          )}
        </div>
        ...
        <div className="px-4 pb-3 flex gap-3">
          {timerState === 'running' ? (
            <button
              onClick={handleStop}
              className="flex-1 py-3.5 bg-red-600 text-white rounded-xl text-lg font-bold active:bg-red-700 transition"
            >
              Stop
            </button>
          ) : (
            <button
              ...
              className="flex-1 py-3.5 bg-green-600 text-white rounded-xl text-lg font-bold active:bg-green-700 disabled:opacity-40 transition"
            >
          ...
          {timerState === 'stopped' && (
            <button
              onClick={handleReset}
              className="py-3.5 px-5 bg-gray-700 text-white rounded-xl text-lg font-bold active:bg-gray-600 transition"
            >
              Reset
            </button>
          )}
      ...
      <div className="px-4 py-3 border-b border-gray-800" style={{ backgroundColor: '#1a2332' }}>
        <div className="flex gap-2">
          <input
            ...
            placeholder="Bib #"
            autoFocus
            className="flex-1 bg-gray-800 text-white border border-gray-600 rounded-xl px-4 py-3 text-2xl font-mono text-center focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-gray-500"
          />
          <button
            onClick={submitBib}
            disabled={submitting || !matchedAthlete}
            className="px-5 py-3 bg-blue-600 text-white rounded-xl text-base font-bold active:bg-blue-700 disabled:opacity-40 transition whitespace-nowrap"
          >
            Log
          </button>
        </div>
        ...
        {bibInput.trim() && (
          <div className="mt-2 px-1">
            {matchedAthlete ? (
              ...
              <div className="text-gray-500 text-sm">No match</div>
            )}
          </div>
        )}
        {error && (
          <div className="mt-2 px-3 py-2 bg-red-900/50 text-red-300 rounded-lg text-sm font-medium">
            {error}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
          ...
          <div>
            ...
            <div className="space-y-2">
              {captures.map((c, i) => (
                ...
                >
                  <div className="text-lg font-bold text-gray-500 w-7 text-center shrink-0">
                    {c.rank}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs bg-gray-700 px-1.5 py-0.5 rounded">{c.bib}</span>
                      <span className="font-medium truncate">{c.athleteName}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="font-mono text-lg font-bold">{c.time}</span>
                    {qualBadge(c.qual_status)}
                  </div>
                  <button
                    onClick={() => handleDelete(c.id, i)}
                    className="text-gray-500 active:text-red-400 p-1 shrink-0"
                    aria-label="Delete"
                  >
        ...
        {captures.length === 0 && timerState === 'running' && (
          <div className="text-center py-8 text-gray-500">
            <p>Type bib numbers as athletes finish</p>
          </div>
        )}

        {timerState === 'idle' && (
          <div className="text-center py-12 text-gray-500">
            <p className="text-lg mb-1">Select an event and press Start</p>
            <p className="text-sm">Type bib numbers as athletes cross the finish line</p>
          </div>
        )}
      </div>
    </div>
